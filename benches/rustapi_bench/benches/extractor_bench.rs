...
fn bench_query_extraction(c: &mut Criterion) {
    ...
    group.bench_function("simple_query", |b| {
        b.iter(|| {
            serde_urlencoded::from_str::<SimpleQuery>(black_box(simple_query)).unwrap()
        })
    });
    
    // Complex query
    let complex_query = "page=1&limit=10&sort=created_at&filter=active&include=posts&include=comments";
    group.bench_function("complex_query", |b| {
        b.iter(|| {
            serde_urlencoded::from_str::<ComplexQuery>(black_box(complex_query)).unwrap()
        })
    });
    ...
    group.bench_function("empty_query", |b| {
        b.iter(|| {
            serde_urlencoded::from_str::<SimpleQuery>(black_box(empty_query)).unwrap()
        })
    });
...
fn bench_json_extraction(c: &mut Criterion) {
    ...
    group.bench_function("simple_body", |b| {
        b.iter(|| {
            serde_json::from_str::<UserBody>(black_box(simple_json)).unwrap()
        })
    });
    ...
    group.bench_function("complex_body", |b| {
        b.iter(|| {
            serde_json::from_str::<ComplexBody>(black_box(complex_json)).unwrap()
        })
    });
    ...
    group.bench_function("large_array_body", |b| {
        b.iter(|| {
            serde_json::from_str::<Vec<UserBody>>(black_box(&large_json)).unwrap()
        })
    });
