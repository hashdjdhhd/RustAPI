...
fn bench_text_message(c: &mut Criterion) {
    ...
    for (name, msg) in messages.iter() {
        ...
        group.bench_with_input(
            ...
            msg,
            |b, msg| b.iter(|| parse_text_message(black_box(msg))),
        );
    }
...
fn bench_binary_message(c: &mut Criterion) {
    ...
    for (name, msg) in messages.iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("parse", name),
            msg,
            |b, msg| b.iter(|| parse_binary_message(black_box(msg))),
        );
    }
...
fn bench_json_message(c: &mut Criterion) {
    ...
    let chat_json = r#"{"type":"message","user":"alice","content":"Hello everyone!","timestamp":1704067200}"#;
...
fn bench_connection_management(c: &mut Criterion) {
    ...
    group.bench_function("list_room_members", |b| {
        ...
        b.iter(|| {
            rooms.get(black_box("general")).map(|members| members.len())
        })
    });
