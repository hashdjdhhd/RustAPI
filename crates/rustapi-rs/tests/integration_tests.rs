...
mod router_tests {
    ...
    fn test_router_method_routing() {
        ...
        let found = routes.iter().any(|r| 
            r.path() == "/integ-method-test" && r.method() == "GET"
        );
    ...
    async fn user_handler(
        rustapi_rs::Path(user_id): rustapi_rs::Path<i64>
    ) -> String {
        format!("user={}", user_id)
    }
...
mod openapi_tests {
    ...
    fn test_schema_generation() {
        ...
        assert_eq!(name, "IntegApiResponse", "Schema name should match struct name");
    }
...
mod compression_tests {
    ...
    fn test_content_type_check() {
        let compressible = ["text/html", "application/json", "text/css", "text/javascript"];
